...
The data needed for analysis is downloaded from [data.gov](https://catalog.data.gov/dataset/health-conditions-among-children-under-age-18-by-selected-characteristics-united-states-53b56). The Centers for Disease Control and Prevention (CDC) published the original data in Jun 2021[@healthc2023].

This data set consists of information about the health conditions among children under 18 years old based on different population characteristics. It can help get information regarding the prevalent illness among minors, the percentage of children for each disease based on the various demographic characteristics, and more.
...
About the percentage (or estimate) column, it might be good to keep it like this without calculating the percent (or dividing by 100), as the parameter provided from the table is the percentage. However, it might be necessary to divide 100 in future analysis when correcting the axle scale name for the graphic.
...
health_clean <- health %>%
  ...
  rename("illness"=`PANEL`, "characteristics"=`STUB_NAME`, "sub_name"=`STUB_LABEL`, "year"= `YEAR`, "year_number"= `YEAR_NUM`, "percentage"=`ESTIMATE`, "se"=`SE`) %>%
   mutate(illness= str_remove(illness, "among persons under 18 years"))%>% # Make the information contain in this column more concise.
...
## Summary of the characteristic variables
...
```{r}
health_clean %>% select(where(is.character)) %>% colnames()

freq_sumary_table_illness <-health_clean %>%
   count(illness)%>%
   mutate(prop= sum(n)/n)%>%
   mutate(prop= prop/100)%>%
   mutate(prop=scales::percent(prop)) # Changing the number into the percentage makes the table more straightforward.
...
## Summary of the numerical variables- percentage

To summarize the numerical variables "percentage" (or percent of the students), grouping the variable into the different demographic categories based on various illnesses is necessary. The summary includes information about the central tendency of the percentage, including the mean and median. It aims to describe the center of distribution of data[@seanconway]. The summary also includes range, standard deviation, maximum, minimum, and quartile information. It shows the variability or the differences between values in this numerical variable and can determine whether or not the variable "percentage" contains the extreme variable[@seanconway] [@3.2-id].
...
```{r}
q = c(.25, .5, .75) # The parameter used for calculating the quartile. 
numerical_summary_table <- function(x) {
  health_clean%>%
  filter(str_detect(illness,x)) %>% # filter different illness
...
mutate(IQR=quant75- quant25,extrem_value_range_percentage_max= quant75 + (1.5 * IQR),extrem_value_range_percentage_min= quant25-(1.5 * IQR)) # Calculate the outlier.

} 
 
illness_name <- unique(health_clean$illness) # Show the various illness names.

illness_name

data_frame <- map(illness_name,numerical_summary_table)# List the summary table for "percentage" based on various illnesses.

data_frame

ADHD_numerical_summary <- data_frame[1] %>% as.data.frame() %>% mutate(Illness= "ADHD") # Changing the list into a table. Then, add a column that includes the illness name, making the table easier to understand.
...
df%>%
  filter(max_percentage>extrem_value_range_percentage_max) # Filter the maximum percentage for each category larger than the upper limit of the range set up through the IQR method.
  }

find_extrem_value_low<- function(df) {
df%>%
  filter(min_percentage<extrem_value_range_percentage_min) # Filter the minimum percentage for each category smaller than the lower limit of the range set up through the IQR method. 
  }
...
As the "year" is the categorical variable and represents the range of years, it might not be suitable to use it to show the trend of the percentage of children changing over time by using the liner graphic. Instead, the year_number is a numerical variable and includes the information for various year ranges, so using it in future analysis is more appropriate. This section aims to show how the information in the "year" column matches the information in the "year_number" column, which hopes to make the graphics in future sections easier to understand.
...
3.  After displaying the trend of percentage for various illnesses over time, which illness is more prevalent over time? How does the percentage for different demographic characteristics change over time in this particular illness?

4.  After displaying the trend of percentage for various illnesses over time, which percentage of children with illness increased most over time? How does this particular illness's percentage of demographic characteristics change over time?
...
table <- health_clean %>%
  ...
  mutate(sub_name= str_remove(sub_name, ": Medicaid$|: Private$")) # Make the values in the insurance status column only contain "Insured" and "Uninsured."

 Insured <- table %>%
  filter(str_detect(sub_name,"Insured")) %>%
group_by(illness, characteristics, sub_name, year, year_number) %>%
  summarise(percentage= sum(percentage), percentage_high= sum(percentage_high), percentage_low= sum(percentage_low)) # Although the name is displayed as two variables, the percentage values are still displayed as three different variables. Using the "sum()" function to add them together and display them as one variable.
...
As mentioned in the “Overview” section, combining three categories under “Insured” is meaningful. Accordingly, this section uses functions like `str_remove()` and `summaries(sum())` to achieve it.
...
(The graphic shows the trend for various illnesses over time based on insured status.)
...
(The graphic shows the trend for various illnesses over time based on uninsured status.)
...
table_final_illness%>%
  ...
  ungroup()%>%
  summarise(highest_maximum= max(max), highest_minimal=max(min)) # Find the maximum and minimum percentages for different illnesses to see which is most prevalent. 
}
```
...
Insured_graphic+
  gghighlight(percentage<0.54, percentage>0.44)+ #Highlight the previous graphic through the range.
  labs(title = "Most Prevalence Illness Over Time \n - Insured") # "\n" is used for making the title into two lines.
```

(The graphic shows the most prevalent illness based on insured status.)
...
(The graphic shows the most prevalent illness based on uninsured status.)
...
reorder_poverty_level <- health_clean %>%
  filter(str_detect(characteristics , "poverty"))%>%
   mutate(sub_name=factor(sub_name,levels=c("Below 100%", "100%-199%", "200%-399%","400% or more"))) # Reorder the poverty levels.
...
(The graphics above show the trend for various illnesses over time based on poverty level “Below 100%”)
...
(The graphics above show the trend for various illnesses over time based on poverty level “100%-199%”)
...
(The graphics above show the trend for various illnesses over time based on poverty level “200%-399%”)
...
(The graphics above show the trend for various illnesses over time based on poverty level “400% or more”)
...
(The graphics above show the most prevalent illness based on poverty level “Below 100%”)
...
(The graphics above show the most prevalent illness based on poverty level 100%-199%)
...
(The graphics above show the most prevalent illness based on poverty level “200%-399%”)
...
(The graphics above show the most prevalent illness based onpoverty level “400% or more”)
...
(The graphic shows the trend for the percentage of children over time based on various illnesses.)
...
illness_change_over_time+
  ...
  labs(title = "Trends For Various Illness Over Time-Most Prevalent")
```
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Age" based on illness "Hay fever or respiratory allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Sex" based on illness "Hay fever or respiratory allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Race" based on illness "Hay fever or respiratory allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Hispanic origin and race" based on illness "Hay fever or respiratory allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Poverty level" based on illness "Hay fever or respiratory allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "insurance status"\[before combining\]based on illness "Hay fever or respiratory allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "insurance status"\[after combining\]based on illness "Hay fever or respiratory allergy")
...
(The graphics above show the trend for the percentage of children over time with demographic characteristics "Total" based on "Hay fever or respiratory allergy.")

Some observations for the graphics above:

1)  The trends for all characteristics over time are going down, except for the "Two or more races" in the "Race" category.
...
illness_change_over_time+
  ...
  labs(title = "Trends For Various Illness Over Time-Increase Most")
```
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Age" based on illness "Skin allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Sex" based on illness "Skin allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Race" based on illness "Skin allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Hispanic origin and race" based on illness "Skin allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Poverty levels" based on illness "Skin allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "insurance status"\[before combining\]based on illness "Skin allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "insurance status"\[after combining\]based on illness "Skin allergy")
...
(The graphic shows the trend for the percentage of children over time with demographic characteristic "Total" based on illness "Skin allergy")
...
4.  After combining the three sub-categories under the "Insured" status, it shows that the median percentage of children for "Insured" is higher than the median percentage for "Uninsured" status. It might be a good phenomenon, which shows that many children who suffer from those eight diseases are insured. However, it is still possible that the children with the insurance are more likely to go to the hospital, see the doctor, and be diagnosed [@effects].

5.  For the poverty level, the sub-category "Below 100%" has a higher median percentage of children over time compared to other categories, which might show that many children suffering from those eight illnesses are at the "Below 100%" poverty level. Accordingly, this group of children needs more attention.

# Limitation

1.  Some numbers for percentages of children provided through the original data set are considered unreliable[@healthc2019]. However, this project treats all the data as reliable when doing the analysis.
